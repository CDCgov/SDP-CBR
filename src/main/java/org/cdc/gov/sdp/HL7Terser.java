...
public class HL7Terser {
	...
	public boolean filterEqualsWithRepetitions(Exchange exchange) {
		...
		try {
			for (int i = 0; i < MAX_REP; i++) {
				...
				if (org.apache.camel.component.hl7.HL7.terser(filterFieldWithRepetition).isEqualTo(filterValue)
						.matches(exchange)) {
					return true;
				}
			}
	...
	public boolean filterContainsWithRepetitions(Exchange exchange) {
		...
		try {
			for (int i = 0; i < MAX_REP; i++) {
				String filterFieldWithRepetition = filterField.replace("*", Integer.toString(i));
				if (org.apache.camel.component.hl7.HL7.terser(filterFieldWithRepetition).contains(filterValue)
						.matches(exchange)) {
					return true;
				}
			}
