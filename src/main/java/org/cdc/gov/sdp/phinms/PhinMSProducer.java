...
public class PhinMSProducer extends DefaultProducer {
	...
	public void process(final Exchange exchange) throws Exception {
		try {
			if (log_sqlcon == null) {
				log_sqlcon = phinMsDs.getConnection();
			}
			

			List<String> separated = Splitter.on(CharMatcher.anyOf("\n\r")).omitEmptyStrings().splitToList((String)exchange.getIn().getBody());
			...
			if (insertSql == null) {
				insertSql = log_sqlcon.prepareStatement(phinMsInsertSql + " (" + sqlColumns + ") values (" + sqlFields + ")");
			}
			...
			for (int i = 1 ; i < separated.size() ; i++) {
				...
				for (int j = 0; j < values.size(); j++) {
					...
					case "recordId":  // Should not be used in most cases - autogenerated.
						insertSql.setLong(j+1, Long.parseLong(values.get(j)));
						break;
					case "payloadBinaryContent":
						Blob blob = log_sqlcon.createBlob();
