...
public class HL7TerserTest extends SDPTestBase {
	...
	public void testFilterTwoTermsWithRepetition() throws IOException {
		String sourceFile = "src/test/resources/hl7v2_repetitions.txt";
		CamelContext ctx = new DefaultCamelContext();
		Exchange exchange = new DefaultExchange(ctx);
		Message msg = new DefaultMessage();

		msg.setHeader("HL7Filter",
				"*([OBSERVATION(*)/OBX-3-2 CONTAINS Pregnancy] AND [OBSERVATION(*)/OBX-5-1 EQUALS N])");
		msg.setBody(readFile(sourceFile));
		...
		msg.setHeader("HL7Filter",
				"*([OBSERVATION(*)/OBX-3-2 CONTAINS Pregnancy] AND [OBSERVATION(*)/OBX-5-1 EQUALS Y])");
		msg.setBody(readFile(sourceFile));
	...
	public void testFilterComplex() throws IOException {
		...
		msg.setHeader("HL7Filter",
				"[[PID-5-1 EQUALS JONES] AND [PID-5-2 EQUALS MARY]] AND "
