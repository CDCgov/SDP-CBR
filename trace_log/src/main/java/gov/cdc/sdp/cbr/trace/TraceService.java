...
public class TraceService {
   ...
   */
  public TraceLog addTraceMessage(String cbrId, String source, TraceStatus status, String description)
      throws SQLException {
    assert cbrId != null;
    assert status != null;
  ...
  /**
   * Finds and generates the highest severity status across all trace logs for a
   * given CBR ID.
   * 
   * @param cbrId
   *          The CBR ID of the originating message that was sent to CBR.
   ...
   *           Thrown when the specified message has not been logged into the
   *           system.
   * @throws SQLException
  ...
  public TraceStatus getStatus(String cbrId) throws IllegalArgumentException, SQLException, InvalidObjectException {
    ...
    try {
      ...
      ps = conn.prepareStatement("SELECT max(status) from " + tableName + " WHERE CBR_ID = ?");
   ...
   *           Thrown when the status cannot be converted from the tuple
   */
  public TraceStatus getStatus(String cbrId, String source)
      throws IllegalArgumentException, SQLException, InvalidObjectException {

    Connection conn = null;
    PreparedStatement ps = null;
    try {
      conn = traceLogDs.getConnection();
      ps = conn.prepareStatement("SELECT max(status) from " + tableName + " WHERE CBR_ID = ? AND SOURCE = ?");
      ps.setString(1, cbrId);
  ...
  /**
   * Returns an ordered list of all log event for a given inbound message to CBR.
   * 
   * @param cbrId
   *          The CBR ID of the originating message that was sent to CBR.
   ...
   *         found, an empty list
   * @throws SQLException on database error
   * @throws InvalidObjectException Thrown when the status cannot be converted from the tuple
   */
  public List<TraceLog> getTrace(String cbrId) throws SQLException, InvalidObjectException {
    ...
    try {
      conn = traceLogDs.getConnection();
      ps = conn.prepareStatement("SELECT * from " + tableName + " WHERE CBR_ID = ?");
      ps.setString(1, cbrId);
      ResultSet rs = ps.executeQuery();
      ...
      while (rs.next()) {
        ...
        messages.add(new TraceLog(rs.getString("cbr_id"), rs.getString("source"), status, rs.getString("description"),
  ...
  public List<TraceLog> getTrace(String cbrId, String source) throws SQLException, InvalidObjectException {
    ...
    try {
      conn = traceLogDs.getConnection();
      ps = conn.prepareStatement("SELECT * from " + tableName + " WHERE CBR_ID = ? AND SOURCE = ?");
      ps.setString(1, cbrId);
      ps.setString(2, source);
      ResultSet rs = ps.executeQuery();
      List<TraceLog> messages = new ArrayList<TraceLog>();
  ...
  private boolean save(TraceLog log) throws SQLException {
    String sql = "INSERT INTO " + tableName + " (cbr_id, source, status, description, created_at)"
        + " values (?, ?, ?, ?, ?)";
    Connection conn = null;
    PreparedStatement ps = null;
    try {
  ...
  private void setupTable(DataSource ds, String tableName) throws SQLException {
    ...
    String sql = "CREATE TABLE IF NOT EXISTS " + tableName + " (" + "  id           bigserial primary key,"
        + "  cbr_id       varchar(255) NOT NULL, " + "  source       varchar(255) NOT NULL, " + "  status       int, "
        + "  description  varchar(255) NOT NULL, " + "  created_at   timestamp NOT NULL)";
    Connection conn = null;
    PreparedStatement ps = null;
    try {
