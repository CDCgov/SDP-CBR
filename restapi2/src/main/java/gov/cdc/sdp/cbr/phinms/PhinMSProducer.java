...
public class PhinMSProducer extends DefaultProducer {
  ...
  public void process(final Exchange exchange) throws Exception {
    try {
      ...
      if (insertSql == null) {
        insertSql = connection.prepareStatement(phinMsInsertSql + " (" + sqlColumns + ") values (" + sqlFields + ")");
      }
        ...
        for (int j = 0; j < values.size(); j++) {
          switch (headers.get(j)) {
            ...
            case "payloadBinaryContent":
              ...
              } catch (java.sql.SQLFeatureNotSupportedException sqle) {
                insertSql.setBytes(j + 1, values.get(j).getBytes());
                log.warn("Error attempting to create blob data", sqle);
              }
